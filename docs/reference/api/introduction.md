...
5. [Authentication](security/authentication) - Validate JWT and attach the user profile to `req.user` or send a 401 error response<sup>*</sup>.
6. [Access Control](security/authorization) - Determine whether the requester has sufficient permissions to perform the desired operation on a particular resource. Attach the permission object to `req.permission` or send a 403 error response.
7. [Request Validation](core/validation) - Validate if the request query, parameters, or body is in the expected format, or send a 400 error response.
8. [Async Handler](core/error-handling.html#asynchronous-error-handler) - Wrap the business logic route middleware to catch asynchronous errors and propagate them to the global error handler.
...
10. [404 Handler](core/error-handling.html#_404-handler) - Handle routing failures and send a 404 error response.
11. [Custom Error Handlers](core/error-handling.html#custom-error-handlers) - Handle prisma, assertions, axios errors and send appropriate error responses.
12. [Global Error Handler](core/error-handling.html#the-default-error-handler) - Handle all other errors and send a 500 error response.
