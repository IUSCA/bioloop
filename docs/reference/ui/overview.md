...
<script setup>
  ...
  const colorByStatus = status == 'PERMANENTLY_FAILED' ? colors.danger : color.primary
</script>
...
Users are authenticated using IU CAS. [More on auth module](auth-explained.md).
...
Vue Components developed in house to be reused in the app. [Documentation](util-components.md)
