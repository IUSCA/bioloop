...
- **API** `queryService.js` — `isPrivilegedNotificationViewer(user)` centralizes admin|operator; used in `resolveNotificationForUser` (broadcast meta, withdrawal fields, resolver identity) and re-exported for routes.
...
- **Route** withdraw guard now calls `isPrivilegedNotificationViewer(req.user)` instead of inline `hasRole` pairs.
...
- **Route file** still contains large inline `asyncHandler` blocks (POST create, PATCH recipients, mark-all-read, bulk delete, withdraw). Further extraction to e.g. `notificationCommandService.js` would shrink the router further.
