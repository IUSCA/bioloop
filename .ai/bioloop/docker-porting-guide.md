...
`bin/reset_docker.sh` must remove this file so a full environment reset triggers a
re-seed.
...
## 9. `bin/reset_docker.sh` (new file)
...
**What to create:** `bin/reset_docker.sh` — a Bash script that performs a full
environment reset in one command.  The script must be committed to the repository.

**Usage:**
```bash
bin/reset_docker.sh              # interactive — prompts before each destructive step
bin/reset_docker.sh --no-confirm # non-interactive — skips all prompts (CI / scripted use)
...
| 9a | `bin/reset_docker.sh` *(new file)* | Tooling | No automated way to reset environment |
