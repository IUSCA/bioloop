...
router.get(
  ...
  validate([
    ...
    query('session').toBoolean().default(false),
...
router.post(
  ...
  validate([
    ...
    body('session_id').isInt().toInt().optional(),
  ]),
  asyncHandler(async (req, res, next) => {
    // #swagger.tags = ['datasets']
...
router.post(
  ...
  validate([
    param('id').isInt().toInt(),
    param('wf').isIn(['stage', 'integrated', 'source_to_raw_conversion']),
  ]),
  (req, res, next) => {
    // admin and operator roles can run all workflows
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (['stage', 'source_to_raw_conversion'].includes(req.params.wf)) {
      try {
