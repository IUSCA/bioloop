...
async function resolveDynamicArgumentValue(arg, datasetId) {
  ...
  if (arg.dynamic_variable_name === 'SUBJECT_ID') {
    const subject_id_key = dataset?.session?.protocol?.subject_id_key;
    const subject_id = subject_id_key ? dataset?.session?.subject?.[subject_id_key] : null;
    return subject_id ? `sub-${subject_id}` : null;
  }
  if (arg.dynamic_variable_name === 'SESSION_NAME') {
    const study_time = dataset?.session?.study_time?.slice(0, 6);
    const study_date = dataset?.session?.study_date;
    const session_name = `${study_date}T${study_time}`;
