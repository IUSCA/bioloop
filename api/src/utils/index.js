...
/**
 * Safely handles a promise by returning an array with either the resolved data or the error.
 * This function helps avoid using try-catch blocks when working with async/await.
 *
 * @param {Promise} promise - The promise to be resolved or rejected.
 * @returns {Promise<[Error|null, any|null]>} A promise that resolves to an array where the first element
 * is the error (if any) or null, and the second element is the resolved data or null.
 */
async function safeAwait(promise, { logError = false } = {}) {
  try {
    const data = typeof promise.then === 'function' ? await promise : promise;
    return [null, data];
  } catch (error) {
    if (logError) {
