...
//
// if (duplicate_dataset_latest_state !==
...
// { throw new Error(`Expected duplicate dataset ${duplicate_dataset.id} to be
// in one of states ` +
// `${config.DATASET_STATES.DUPLICATE_ACCEPTANCE_IN_PROGRESS} or
// ${config.DATASET_STATES.DUPLICATE_ACCEPTED}, but current state ` + `is
...
// if (original_dataset_latest_state !==
// config.DATASET_STATES.ORIGINAL_DATASET_RESOURCES_PURGED &&
// original_dataset_latest_state !== config.DATASET_STATES.OVERWRITTEN) { throw
// new Error(`Expected original dataset ${original_dataset.id} to be in one of
// states ` + `${config.DATASET_STATES.ORIGINAL_DATASET_RESOURCES_PURGED} or
// ${config.DATASET_STATES.OVERWRITTEN}, but current state is
...
//
//   // update_queries = update_queries.concat(await
//   // audit_and_state_update_queries({ dataset_id: duplicate_dataset_id, state:
//   // config.DATASET_STATES.DUPLICATE_ACCEPTED, }));
//
//   // update_queries.push(prisma.dataset.update({
//   //   where: {
//   //     id: original_dataset.id,
...
//
//   // update_queries = update_queries.concat(await
//   // audit_and_state_update_queries({ dataset_id: original_dataset.id, state:
//   // config.DATASET_STATES.OVERWRITTEN, }));
//
