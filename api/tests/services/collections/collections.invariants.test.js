...
describe('collections - invariants', () => {
  ...
  describe('archived collection blocks mutations', () => {
    ...
    it('removeDatasets on archived collection throws 409', async () => {
      ...
      await collectionsService.addDatasets(c.id, { dataset_resource_ids: [ownDs.resource_id], actor_id: actor.subject_id });
      await collectionsService.archiveCollection(c.id, actor.subject_id);

      await expect(
        collectionsService.removeDatasets(c.id, { dataset_resource_ids: [ownDs.resource_id], actor_id: actor.subject_id }),
      ).rejects.toMatchObject({ status: 409 });
