...
describe('access requests - lifecycle', () => {
  ...
  describe('access requests with presets', () => {
    ...
    // Create a fresh subject for each test to avoid cross-contamination from grants/requests
    let presetSubject;
    beforeEach(async () => {
      presetSubject = presetRequester; // Use same person as requester (self-request)

      // Clean up any old requests and grants for presetSubject on the presetDataset
      try {
        // Try to delete items first (in case there are foreign keys)
        ...
        for (const req of requests) {
          ...
          await prisma.access_request_item.deleteMany({
            where: { access_request_id: req.id },
          });
        }
        ...
        // Ignore errors if clean up fails
    ...
    describe('submitRequest validation with presets', () => {
      it('validates expanded access types for active grants', async () => {
        ...
        await expect(
          arService.submitRequest(request.id, presetRequester.subject_id),
      ...
      it('validates expanded access types against in-flight requests', async () => {
        ...
        await expect(
          arService.submitRequest(request2.id, presetRequester.subject_id),
        ).rejects.toMatchObject({
          status: 409,
        });
      });
