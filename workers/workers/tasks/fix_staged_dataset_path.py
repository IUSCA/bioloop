...
def fix_staged_dataset_path(celery_task, dataset_id, **kwargs):
    ...
    print(f'Downloading {sda_archive_path} to {str(downloaded_sda_bundle_path)}')
    wf_utils.download_file_from_sda(sda_archive_path, downloaded_sda_bundle_path)
    print(f'Downloaded {sda_archive_path} to {str(downloaded_sda_bundle_path)}')

    (staging_dir) = compute_staging_path(dataset)

    print(f'Extracting {str(downloaded_sda_bundle_path)} to {str(staging_dir)}')
