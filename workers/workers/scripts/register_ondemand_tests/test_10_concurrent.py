#!/usr/bin/env python3
"""
Test Case 10: Concurrent Execution

Purpose: Test parallel operations with multiple subdirectories
Data: 5 subdirectories with 2 files each (~10MB total)
Expected: Hard linking with renaming, parallel verification of 5 directories

This test verifies:
- Parallel processing of 5 subdirectories
- Worker configuration and limits (4 workers used)
- Efficient scaling with multiple directories
- Hard-link creation and verification
- Performance benefits of parallelization

Usage:
    python test_10_concurrent.py [--dry-run] [--skip-checksum]
"""
...
class TestCase10:
    """Test Case 10: Concurrent Execution - Testing parallel operations with multiple subdirectories"""
    
    def __init__(self, dry_run: bool = True, skip_checksum: bool = False):
        self.test_name = "Concurrent Execution"
        self.test_id = "10"
        self.description = "Testing parallel operations with multiple subdirectories"
        self.data_dir = "test_10_concurrent"
        self.subdirs = [
            "subdir_parallel_1", "subdir_parallel_2", "subdir_parallel_3", 
            "subdir_parallel_4", "subdir_parallel_5"
        ]
        self.dataset_type = "DATA_PRODUCT"
        self.ingest_subdirs = True
        self.prefix = "PARALLEL"
        self.suffix = "TEST"
        self.expected_hard_links = 5
    ...
    def run_test(self) -> bool:
        ...
        try:
            ...
            worker_config = "Worker Configuration" in output
            parallel_summary = "Parallel Verification Summary" in output
            ...
            test_success = test_success and worker_config  # Should show worker configuration
            test_success = test_success and parallel_summary  # Should show parallel summary
            ...
            print(f"      Worker Configuration: {'✅' if worker_config else '❌'} (expected: True)")
            print(f"      Parallel Summary: {'✅' if parallel_summary else '❌'} (expected: True)")
            print(f"      Checksum Verification: {'✅' if checksum_verification else '❌'}")
            print(f"      Cleanup Executed: {'✅' if cleanup_executed else '❌'}")
            
            # Show parallel verification details
            if parallel_verification:
                print(f"    🔍 Parallel Verification Details:")
                lines = output.split('\n')
                for line in lines:
                    if any(keyword in line for keyword in [
                        "Worker Configuration", "Starting parallel verification", 
                        "Parallel verification completed", "Parallel Verification Summary"
                    ]):
                        print(f"      {line.strip()}")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Test Case 10: Concurrent Execution")
    ...
    # Create and run test
    test = TestCase10(dry_run=dry_run, skip_checksum=args.skip_checksum)
