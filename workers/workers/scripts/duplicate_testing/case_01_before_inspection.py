"""
Case 01 — Duplicate registered BEFORE original reaches INSPECTED state.

Scenario
--------
Both the original and the duplicate are registered in rapid succession (within
a few seconds of each other), before either has been inspected by the worker.

When the duplicate's inspect_dataset task runs, it calls
wait_for_concurrent_inspections(), which detects that the original was created
before it and has not yet reached INSPECTED.  It polls (up to
concurrent_inspection_wait_timeout_seconds) until the original's inspection
completes.  Only then does Jaccard duplicate detection run, finding the original
with score = 1.0.
...
Naming convention
-----------------
  original dir and API name: dup-test--case-01--<timestamp>  (unique-checked)
...
def run() -> None:
    ...
    logger.info('=' * 70)
    logger.info('Case 01: duplicate registered BEFORE original is INSPECTED')
    logger.info(f'  original name:  {original_name}')
    logger.info(f'  files:          {SHARED_FILES}')
    logger.info(f'  Jaccard:        {jaccard_note(5, 5, 5)}')
    logger.info('=' * 70)
    logger.info(
        'NOTE: await_stability takes ~1 min in Docker (recency_threshold_seconds=60). '
        'Both datasets are registered back-to-back; the duplicate worker will '
        'automatically wait for the original to be INSPECTED before running '
        'duplicate detection.'
    )

    # Step 1 — Create original files and register the original dataset.
    logger.info('')
    logger.info('Step 1: creating original directory and registering dataset...')
    make_dataset_dir(original_dir, SHARED_FILES)
    original = register_and_start(original_name, original_dir)

    # Step 2 — Derive a unique duplicate name that encodes the original's ID and
    ...
    logger.info('')
    logger.info('Step 2: creating duplicate directory and registering (no wait)...')
    ...
    # Wait for the original to be INSPECTED (signals the duplicate worker can proceed).
    logger.info('')
    logger.info('Step 3: waiting for original to reach INSPECTED (~5-8 min)...')
    ...
    # Wait for the duplicate to be registered as a duplicate.
    logger.info('')
    logger.info('Step 4: waiting for duplicate to reach DUPLICATE_REGISTERED...')
    reached = wait_for_state(
