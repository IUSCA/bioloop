#!/usr/bin/env python3
"""
Test Case 8: DATA_PRODUCT with suffix parameter

This test creates multiple datasets, runs register_ondemand with a suffix,
and verifies that all created datasets have the suffix at the end of their names.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/opt/sca/logs/register_ondemand/test_case_08.log'),
...
def run_test():
    """Run test case 8: DATA_PRODUCT with suffix parameter"""
    logger.info("=" * 60)
    logger.info("Starting Test Case 8: DATA_PRODUCT with suffix parameter")
    logger.info("=" * 60)
    ...
    try:
        ...
        # Step 3: Define suffix and expected dataset names
        test_suffix = "SUFFIX456"
        expected_names = [f"{name}-{test_suffix}" for name in original_subdirs]
        logger.info(f"Using suffix: {test_suffix}")
        logger.info(f"Expected dataset names: {expected_names}")
        
        # Step 4: Run register_ondemand with suffix
        logger.info("Step 4: Running register_ondemand script with suffix...")
        cmd = [
        ...
        # Step 7: Verify that original names (without suffix) are NOT in database
        logger.info("Step 7: Verifying that unsuffixed names are NOT in database...")
        ...
        if result.returncode == 0:
            logger.info("✅ Test Case 8 PASSED: Command executed successfully")
        else:
            logger.error("❌ Test Case 8 FAILED: Command returned non-zero exit code")
        
    except Exception as e:
        logger.error(f"❌ Test Case 8 FAILED with exception: {e}")
