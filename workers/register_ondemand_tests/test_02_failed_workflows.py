#!/usr/bin/env python3
"""
Test Case 02: Previously Failed Workflows

Purpose: Test behavior when datasets have previously failed workflows
Data: 2 subdirectories with 2 files each (~4MB total)
Expected: Script should handle failed workflows gracefully and continue processing

This test verifies:
- Behavior when datasets exist but have failed workflow states
- Script continues processing other candidates
- Failed workflow datasets are logged appropriately
- No crashes or unexpected behavior

Usage:
    python test_02_failed_workflows.py [--dry-run] [--skip-checksum]
"""
...
class TestCase02:
    """Test Case 02: Previously Failed Workflows - Testing behavior with failed workflow states"""
    
    def __init__(self, dry_run: bool = True, skip_checksum: bool = False):
        self.test_name = "Previously Failed Workflows"
        self.test_id = "02"
        self.description = "Testing behavior when datasets have previously failed workflows"
        self.data_dir = "test_02_failed_workflows"
        self.subdirs = ["subdir_failed_1", "subdir_failed_2"]
        self.dataset_type = "DATA_PRODUCT"
        self.ingest_subdirs = True
        self.prefix = "FAILED"
        self.suffix = "WORKFLOW"
    ...
    def check_dataset_exists(self, dataset_name: str) -> bool:
        ...
        try:
            ...
            if exists:
                ...
                workflow_states = [state['state'] for state in dataset.get('states', [])]
                print(f"    📊 Current workflow states: {workflow_states}")
                
                # Check for failed states
                failed_states = [s for s in workflow_states if 'FAILED' in s or 'ERROR' in s]
                if failed_states:
                    print(f"    ⚠️  Dataset has failed workflow states: {failed_states}")
                    print(f"    ℹ️  This will test failed workflow handling")
                else:
                    print(f"    ℹ️  Dataset exists but no failed states - will test normal processing")
            else:
    ...
    def run_test(self) -> bool:
        ...
        try:
            ...
            # Check for workflow-related messages
            workflow_messages = any(keyword in output for keyword in [
                "workflow", "workflow", "Integrated", "kicked off", "already exists"
            ])
            ...
            print(f"      Workflow Messages: {'✅' if workflow_messages else '❌'}")
            
            # Show workflow-related details
            if workflow_messages:
                print(f"    🔍 Workflow-Related Details:")
                lines = output.split('\n')
                for line in lines:
                    if any(keyword in line.lower() for keyword in [
                        "workflow", "integrated", "kicked off", "already exists", "failed", "error"
                    ]):
                        print(f"      {line.strip()}")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Test Case 02: Previously Failed Workflows")
    ...
    # Create and run test
    test = TestCase02(dry_run=dry_run, skip_checksum=args.skip_checksum)
