...
Expected: Hard linking with renaming, parallel verification of 2 directories

This test verifies:
- Prefix/suffix renaming functionality
- Hard-link creation for renamed directories
- Parallel verification of multiple directories
- Directory-level checksum verification
- Proper cleanup of hard-linked artifacts

Usage:
    python test_02_renaming_with_prefix_suffix.py [--dry-run] [--skip-checksum]
"""
...
try:
    ...
    print("Make sure you're running this from the the correct directory")
...
class TestCase02:
    ...
    def __init__(self, dry_run: bool = True, skip_checksum: bool = False):
        self.test_name = "Renaming with Prefix/Suffix"
        self.test_id = "02"
        self.description = "Renaming functionality with prefix/suffix and parallel verification"
        self.data_dir = "test_02_renaming_with_prefix_suffix"
        self.subdirs = ["subdir_alpha", "subdir_beta"]
        self.dataset_type = "DATA_PRODUCT"
        self.ingest_subdirs = True
        self.prefix = "TEST_PRE"
        self.suffix = "TEST_SUF"
        ...
        self.file_sizes_mb = [2, 2]  # 2 files of 2MB each per subdirectory
        ...
        self.expected_dataset_names = [
            ...
            f"{self.prefix}-{self.subdirs[1]}-{self.suffix}"
        ]
    ...
    def run_test(self) -> bool:
        ...
        try:
            ...
            if "Parallel verification" in output:
                print(f"    🔍 Parallel Verification Details:")
                lines = output.split('\n')
                for line in lines:
                    if "Parallel verification" in line or "Worker Configuration" in line or "verification completed" in line:
                        print(f"      {line.strip()}")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Test Case 02: Renaming with Prefix/Suffix")
