#!/usr/bin/env python3
"""
Test Case 04: API Failure Scenarios

Purpose: Test behavior when dataset registration API calls fail
Data: 2 subdirectories with 2 files each (~4MB total)
Expected: Script should handle API failures gracefully and continue processing

This test verifies:
- Behavior when API calls fail (network issues, server errors)
- Graceful degradation and error handling
- Logging of API failures
- Continuation of processing for other candidates

Usage:
    python test_04_api_failures.py [--dry-run] [--skip-checksum]
"""
...
class TestCase04:
    """Test Case 04: API Failure Scenarios - Testing behavior with API failures"""
    
    def __init__(self, dry_run: bool = True, skip_checksum: bool = False):
        self.test_name = "API Failure Scenarios"
        self.test_id = "04"
        self.description = "Testing behavior when dataset registration API calls fail"
        self.data_dir = "test_04_api_failures"
        self.subdirs = ["subdir_api_1", "subdir_api_2"]
        ...
        self.test_data_base = Path("/opt/sca/app")
    ...
    def run_test(self) -> bool:
        ...
        try:
            ...
            api_messages = any(keyword in output for keyword in [
                "api", "registration", "dataset", "workflow", "kicked off"
            ])
            ...
            print(f"      API Messages: {'✅' if api_messages else '❌'}")
            
            # Show API-related details
            if api_messages:
                print(f"    🔍 API-Related Details:")
                lines = output.split('\n')
                for line in lines:
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Test Case 04: API Failure Scenarios")
    ...
    # Create and run test
    test = TestCase04(dry_run=dry_run, skip_checksum=args.skip_checksum)
