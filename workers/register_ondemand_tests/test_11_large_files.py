#!/usr/bin/env python3
"""
Test Case 11: Large Files

Purpose: Test with larger individual files
Data: 1 subdirectory with 2 files of 10MB each (~20MB total)
Expected: Hard linking with renaming, single worker verification

This test verifies:
- Large file handling (10MB files)
- Single worker verification (no parallel needed)
- Hard-link creation for renamed directories
- Performance with larger file sizes

Usage:
    python test_11_large_files.py [--dry-run] [--skip-checksum]
"""
...
class TestCase11:
    """Test Case 11: Large Files - Testing with larger individual files"""
    
    def __init__(self, dry_run: bool = True, skip_checksum: bool = False):
        self.test_name = "Large Files"
        self.test_id = "11"
        self.description = "Testing with larger individual files"
        self.data_dir = "test_11_large_files"
        self.subdirs = ["subdir_large_files"]
        ...
        # Test data configuration - larger files
        self.file_sizes_mb = [10, 10]  # 2 files of 10MB each
    ...
    def create_test_data(self) -> bool:
        ...
        print(f"    ⚠️  Creating large files - this may take a moment...")
            ...
            for i, size_mb in enumerate(self.file_sizes_mb):
                ...
                with open(file_path, 'wb') as f:
                    for chunk in range(size_mb):
                        ...
                        if chunk % 5 == 0:  # Progress indicator every 5MB
    ...
    def run_test(self) -> bool:
        ...
        try:
            ...
            single_verification = "Verifying hard-links between" in output
            ...
            test_success = test_success and parallel_verification  # Should use parallel verification (even for 1 dir)
            test_success = test_success and single_verification  # Should also show single verification details
            ...
            print(f"      Parallel Verification: {'❌' if parallel_verification else '✅'} (expected: False)")
            print(f"      Single Verification: {'✅' if single_verification else '❌'} (expected: True)")
            ...
            if verification_executed:
                ...
                for line in lines:
                    if any(keyword in line for keyword in [
                        "Verifying hard-links between", "Directory checksums match",
                        "Directory checksum verification passed", "verification completed"
                    ]):
                        print(f"      {line.strip()}")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Test Case 11: Large Files")
    ...
    # Create and run test
    test = TestCase11(dry_run=dry_run, skip_checksum=args.skip_checksum)
