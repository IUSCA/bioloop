<template>
  <div
    ...
    :class="[
      ...
      { unread: !props.notification.is_read },
    ]"
  >
    <!-- Icon -->
    <div class="notif-icon">
      <Icon
        :icon="TYPE_ICONS[props.notification.type] ?? TYPE_ICONS.fallback"
        class="text-lg"
      />
    </div>
    ...
    <div class="notif-body min-w-0 flex-1">
      ...
      <div class="notif-meta mt-1.5">
        ...
        <span class="notif-time">{{
          fromNow(props.notification.created_at)
        }}</span>
      </div>
    </div>
...
.type-alert .notif-icon {
  background: rgba(228, 34, 34, 0.1);
  color: var(--va-danger);
}
.type-workflow .notif-icon {
  background: rgba(21, 78, 193, 0.1);
  color: var(--va-primary);
}
.type-request .notif-icon {
  background: rgba(255, 212, 58, 0.12);
  color: #b38600;
}
.type-system .notif-icon {
  background: rgba(21, 141, 227, 0.1);
  color: var(--va-info);
}

.dark .type-request .notif-icon {
...
.type-alert .notif-type-tag {
  background: rgba(228, 34, 34, 0.1);
  color: var(--va-danger);
}
.type-workflow .notif-type-tag {
  background: rgba(21, 78, 193, 0.1);
  color: var(--va-primary);
}
.type-request .notif-type-tag {
  background: rgba(255, 212, 58, 0.12);
  color: #b38600;
}
.type-system .notif-type-tag {
  background: rgba(21, 141, 227, 0.1);
  color: var(--va-info);
}

.dark .type-request .notif-type-tag {
  color: var(--va-warning);
}
