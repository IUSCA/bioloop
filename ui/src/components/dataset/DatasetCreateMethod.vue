<template>
  <div class="flex items-center">
    ...
    <va-popover v-if="tooltipParts" placement="right">
      <template #body>
        <div class="w-96">
          {{ tooltipParts.prefix }}<code class="mx-1 px-1 rounded text-sm font-mono bg-black/10 dark:bg-white/10">{{ props.originPath ?? '(unknown path)' }}</code>{{ tooltipParts.suffix }}
        </div>
      </template>
      <va-icon name="info_outline" size="small" class="ml-1 va-text-secondary cursor-help" />
    </va-popover>
  </div>
</template>
...
const TOOLTIP_PARTS = {
  IMPORT:    { prefix: 'Directory ', suffix: ' was selected for registration, using the web-portal' },
  SCAN:      { prefix: 'Directory ', suffix: " was found to be eligible for registration by Bioloop's automated-watchers" },
