...
import { OPERATOR_STORAGE_STATE } from '../../../playwright.paths';

// import config from 'config';
const config = require('config');

setup('login', async ({ page }) => {
  await page.goto(`${config.baseURL}/auth/iucas?ticket=operator`, { waitUntil: 'domcontentloaded' });
  ...
  expect(userEntry?.value).toContain(config.e2e.users.operator.username);
